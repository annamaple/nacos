...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void registerInstance(String namespaceId, String serviceName, Instance instance) throws NacosException {
        //1、创建空的service【这里面除了实例没有设置进去，其他都已经设置进去了，命名空间，组，服务，集群】
        // namespaceId:public
        // serviceName:DEFAULT_GROUP@@msb-order   组名@@项目名称
    ...
    public void addInstance(String namespaceId, String serviceName, boolean ephemeral, Instance... ips)
        ...
        synchronized (service) {
            ...
            //将service对应的全量实例
            //instances写入内存注册表
            //一致性service的map，在这里我们同样发现有好几个实现，还是通过寻找定义的地方，发现实现应为DelegateConsistencyServiceImpl这个实现
        ...
        if (!serviceMap.containsKey(service.getNamespaceId())) {
            synchronized (putServiceLock) {
                if (!serviceMap.containsKey(service.getNamespaceId())) {
                    ...
                    //service的结构为Map<namespace, Map<group::serviceName, Service>>，其中，第一层map的key为命名空间，
                    // 第二层map的key为service所在的组名加service名，对应之前的nacos服务注册表结构图
